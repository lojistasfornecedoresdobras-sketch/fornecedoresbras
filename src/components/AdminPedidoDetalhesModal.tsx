...
import { Loader2, Package, Truck, CheckCircle, XCircle, ArrowRight, MapPin, DollarSign, User } from 'lucide-react';
...
interface AdminPedidoDetalhesModalProps {
...
const AdminPedidoDetalhesModal: React.FC<AdminPedidoDetalhesModalProps> = ({ pedidoId, isOpen, onClose, onUpdate }) => {
  ...
  const userIds = [pedido?.lojista_id, pedido?.fornecedor_id].filter((id): id is string => !!id);
  const { userNames: userNamesMap, isLoading: isUserNamesLoading } = useB2BUserNames(userIds);
  
  const lojistaNome = userNamesMap[pedido?.lojista_id || ''] || (pedido?.lojista_id ? `Lojista ID: ${pedido.lojista_id.substring(0, 8)}` : 'N/A');
  const fornecedorNome = userNamesMap[pedido?.fornecedor_id || ''] || (pedido?.fornecedor_id ? `Fornecedor ID: ${pedido.fornecedor_id.substring(0, 8)}` : 'N/A');
  ...
  const isContentLoading = isLoading || isProductNamesLoading || isUserNamesLoading;

  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[650px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-atacado-primary">Gerenciar Pedido #{pedido?.id?.substring(0, 8)}</DialogTitle>
          <DialogDescription>
            Visão administrativa completa do pedido, incluindo status, frete e detalhes financeiros.
          </DialogDescription>
        </DialogHeader>
            ...
            <Card className="bg-gray-50 border-gray-200">
              <CardContent className="p-4 text-sm space-y-1">
                ...
                <p className="flex items-center">
                    ...
                    <strong>Lojista:</strong> <span className="font-semibold ml-1">{lojistaNome}</span>
                </p>
                <p className="flex items-center">
                    <User className="w-4 h-4 mr-1 text-atacado-accent" />
                    <strong>Fornecedor:</strong> <span className="font-semibold ml-1">{fornecedorNome}</span>
                </p>
            ...
            <Card className="border-green-500/50">
              <CardContent className="p-4 space-y-2">
                <h3 className="font-semibold text-green-600 flex items-center">
                  <DollarSign className="w-4 h-4 mr-2" /> Detalhes Financeiros
                </h3>
                {pagamento ? (
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between">
                      <span>Total Bruto do Pedido:</span>
                      <span className="font-bold text-lg">{formatCurrency(pagamento.valor_total)}</span>
                    </div>
                    ...
                    <div className="flex justify-between text-red-500">
                      ...
                      <span className="font-bold">{formatCurrency(pagamento.split_plataforma)}</span>
                    </div>
                    <p className="text-xs text-gray-500 pt-1">Pagamento: {pagamento.status} via {pagamento.metodo} ({pagamento.parcelas}x)</p>
                  </div>
                ) : (
              ...
              <CardContent className="p-4 space-y-3">
                ...
                {frete?.codigo_rastreio ? (
                  ...
                  <form onSubmit={handleSaveFrete} className="space-y-3">
                    <p className="text-sm text-gray-500">Registrar dados de envio (Frete Mock).</p>
...
export default AdminPedidoDetalhesModal;