...
import { Package, Truck, Loader2, ArrowRight, CheckCircle, XCircle, ShoppingBag } from 'lucide-react';
...
const PedidosLojista: React.FC = () => {
  ...
  const fetchPedidos = async () => {
    ...
    setIsLoading(true);
    // Busca pedidos onde o lojista_id é o ID do usuário logado
    const { data, error } = await supabase
      .from('pedidos')
      .select('id, created_at, total_atacado, status, fornecedor_id')
      .eq('lojista_id', user.id)
      .order('created_at', { ascending: false });

    if (error) {
      showError("Erro ao carregar histórico de pedidos: " + error.message);
      console.error(error);
    } else {
  ...
  // Se o usuário for fornecedor, redirecionamos para a página de pedidos de fornecedor
      ...
      <main className="container mx-auto p-4 space-y-6 pt-8">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-bold text-atacado-primary flex items-center">
            <ShoppingBag className="w-7 h-7 mr-3" /> MEUS PEDIDOS ATACADO
          </h1>
        </div>
          ...
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <div className="p-8 text-center text-gray-500">
                Você ainda não realizou nenhum pedido no atacado.
              </div>
            ) : (
              <Table>
                <TableHeader>
